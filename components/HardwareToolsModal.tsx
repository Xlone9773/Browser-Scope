            ...
            {activeTab === 'keyboard' && (
                <div className="h-full flex flex-col animate-in fade-in duration-300 gap-4">
                    ...
                    <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center p-8 shadow-sm">
                        {lastKey ? (
                            <>
                                <div className="text-6xl font-bold text-indigo-600 dark:text-indigo-400 mb-4">{lastKey.key}</div>
                                <div className="flex gap-4 text-xs font-mono text-slate-500">
                                    <div className="bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">Code: {lastKey.code}</div>
                                    <div className="bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded">Time: {lastKey.timestamp}</div>
                                </div>
                            </>
                        ) : (
                            <div className="text-slate-400 text-center">
                                <Keyboard size={48} className="mx-auto mb-2 opacity-20" />
                                <p>{t.key_instruction}</p>
                            </div>
                        )}
                    </div>

                    {/* History */}
                    <div className="bg-slate-100 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700 max-h-[150px] overflow-y-auto">
                        <div className="text-xs font-semibold text-slate-500 uppercase mb-2 sticky top-0 bg-slate-100 dark:bg-slate-800/0 backdrop-blur-sm pb-1">{t.key_history} ({keyHistory.length})</div>
                        <div className="flex flex-wrap gap-2">
                            {keyHistory.map((k) => (
                                <span key={k} className="px-2 py-1 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded text-xs font-mono text-slate-700 dark:text-slate-300 shadow-sm animate-in zoom-in duration-200">
                                    {k}
                                </span>
                            ))}
                        </div>
