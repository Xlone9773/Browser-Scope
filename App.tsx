...
const App: React.FC = () => {
  ...
  const checkPermissionStatus = async (key: PermissionKey, name: string) => {
      try {
          ...
          // Permission query not supported or name invalid for this browser
  ...
  const handleExportJSON = () => {
    ...
    // Add geo data to export if available
    const exportData = {
        ...data,
  ...
  // Helper to translate boolean-like values
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-slate-900 text-slate-800 dark:text-slate-100 py-10 px-4 sm:px-6 lg:px-8 transition-colors duration-300 scrollbar-hide">
      ...
      {isAudioModalOpen && (
          <AudioRecorderModal onClose={() => setIsAudioModalOpen(false)} t={t.audioTool} />
      )}
      {isWebGLExtensionsOpen && (
          <WebGLExtensionsModal 
            extensions={data.fingerprints.webglExtensions || []} 
            onClose={() => setIsWebGLExtensionsOpen(false)} 
            t={t.webglTool} 
          />
      )}
      {isCanvasModalOpen && (
      ...
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex gap-3 relative z-40 flex-wrap">
            ...
            <div className="relative">
                <button 
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all shadow-sm active:scale-95"
                >
                    <Languages size={16} />
                ...
                <div className={`absolute right-0 top-full mt-2 w-48 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl py-1 z-50 transform transition-all duration-200 origin-top-right ${isLangMenuOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none'}`}>
                    {Object.entries(languageNames).map(([code, name]) => (
                        <button
                            key={code}
                            onClick={() => { setLang(code as Language); setIsLangMenuOpen(false); }}
            ...
            <button 
                onClick={fetchData}
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-600 dark:bg-indigo-600 border border-indigo-600 dark:border-indigo-600 rounded-lg text-sm font-medium text-white hover:bg-indigo-700 dark:hover:bg-indigo-500 transition-all shadow-sm active:scale-95"
            >
                <RefreshCw size={16} />
                {t.refresh}
            </button>
        ...
        <div className="pt-4">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                <AppWindow className="text-sky-500" size={24} />
                {t.sections.pwa}
            </h2>
                    ...
                    {data.pwaFeatures.map((feature) => (
                        <div 
                            ...
                            className={`
                                ...
                                ${feature.supported 
                                    ? 'bg-sky-50 dark:bg-sky-900/10 border-sky-100 dark:border-sky-900/30 hover:border-sky-200 dark:hover:border-sky-700 hover:shadow-sm' 
                        ...
                        >
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className={`
                                    ...
                                    ${feature.supported ? 'bg-sky-500 shadow-[0_0_8px_rgba(14,165,233,0.4)]' : 'bg-slate-300 dark:bg-slate-600'}
                                `} />
                            </div>
                    ...
                    {data.features.map((feature) => (
                        <div 
                            ...
                            className={`
                                relative p-4 rounded-lg border transition-all duration-200
                                ${feature.supported 
                                    ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30 hover:border-emerald-200 dark:hover:border-emerald-700 hover:shadow-sm' 
                                    : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700 opacity-70'}
                            `}
                        >
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className={`
                                    w-2 h-2 rounded-full mt-1.5
                                    ${feature.supported ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-slate-300 dark:bg-slate-600'}
                                `} />
                            </div>
                            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                                {/* @ts-ignore */}
